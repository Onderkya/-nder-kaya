...
## 🔁 Revizyon 33 — KANIT KATMANI + üçlü kanal + 4 dil (GÜNCEL · dal `claude/redesign-conversion` · CANLIDA)

> Kullanıcı hedefi: "siteye giren güvensin, inansın, aklında kalalım; satın alma tek net eyleme aksın." Spec `docs/superpowers/specs/2026-07-02-kanit-katmani-design.md` + plan (7 görev, subagent-driven, tüm incelemeler temiz). `tsc` ✓ · `next build` ✓ · canlıda sayfa sayfa doğrulandı. Kısıtlar tutuldu: **hero bayt-bayt aynı, hiçbir mevcut medya silinmedi/değişmedi.**
- **Kanıt katmanı:** ana sayfa fark şeridi ("Acente değiliz…" + 3 mikro-kanıt, `home.diffStrip`); temsili kurucu fotoğrafı (`founders.jpg`, Pexels 17014070, "temsili görsel" ibaresi, kullanıcı onaylı); burs yolculuğu 4→6 adım (`edu_s5` kabul araya, `edu_s6` mezuniyet sona) + her adımda altın "Bu adımda biz" kutusu (`StudyJourney.weDo/weDoLabel`, min-h 27rem); `/lessons`'a A1→C2 seviye yolu (`components/level-path.tsx`, C2'de "Öğretmenimizin seviyesi" rozeti); SSS 4→16 soru (FAQPage JSON-LD otomatik → AI/Google görünürlüğü).
- **Üçlü kanal (satın alma):** her istek noktasında müşteri seçer — **WhatsApp** (önyazılı mesaj) · **Telegram** (paket modalında özet panoya kopyalanır + t.me açılır) · **E-posta** (mailto konu+gövde önyazılı). Yerler: otel kartları, RouteGallery "Bu tatili iste", eğitim CTA, about/faq ConversionBand, ders rezervasyonu altı "randevusuz doğrudan yaz" satırı, iletişim (zaten vardı). Ayarlar canlı DB'de: `WHATSAPP_NUMBER=905340679661`, `TELEGRAM_USERNAME=+905340679661` (t.me/+telefon derin bağlantı; @kullanıcıadı gelirse admin'den değişir, rebuild gerekmez).
- **Admin haritaları:** yeni bölümler/anahtarlar 4 statik haritaya kaydedildi (section-registry: diffStrip+levelPath; asset-slots: education.step5/6.image; editor-map: kartlar + journey 6 adım — `content:[n]` indeks kayması düzeltilip bağımsız doğrulandı; content-map: ~65 yeni anahtar). Kural artık hafızada: **admin otomatik tanımaz, 4 harita elle güncellenir.**
- **Düzeltmeler:** iletişim mesaj kutusu `data-lenis-prevent` (Lenis tekerleği çalıyordu, kutu kaymıyordu) · güvence metni "24 saat" → **"30 dakika içinde kişisel yanıt"** (5 dil).
- **Özbekçe pasif:** site 4 dil (TR·EN·RU·KK) — 5 dosya×8 metin, JSON-LD `knowsLanguage`, 2 sohbet botu talimatı. `uz.json` uykuda (routing zaten 4 dildi); "Özbekistan'dan misafirlere" cümlesi bilinçli kaldı (ülke, dil değil). Yeniden açmak: routing'e `"uz"` + metinleri geri 5'e çıkar.
- **Deploy akışı (bu oturumda işleyen):** local push → sunucuda `git fetch origin claude/redesign-conversion && git merge --ff-only FETCH_HEAD && docker compose up -d --build`. Build-anı ISR önbelleği ~5 dk eski sürümü servis eder — ilk istek arka planda tazeler, ikincisi yenisini alır. ⚠️ Sunucu şifresi sohbette paylaşıldı → değiştir + SSH anahtarına geç. ⚠️ Alan adı hâlâ yok (IP:3010) → DNS + nginx + SSL sıradaki büyük adım (SEO için şart).

## 🔁 Revizyon 32 — FAZ 8: tekil medyalar gizlenebilir (dal `claude/redesign-conversion`)
